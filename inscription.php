    ...
    if(!empty($_POST['email']) && !empty($_POST['password']) && !empty([$_POST['password_two']]) && !empty($_POST['pseudo'])) {
        ...
        while($pseudo_verification = $req->fetch()){
            if($pseudo_verification['numberPseudo'] != 0){
                header('location: inscription.php?error=1&message=Votre pseudo est deja utilisé par un autre utilisateur.');
                exit();
            }
        }
        ...
        // ENVOI
        $req = $db->prepare('INSERT INTO user(email, password, secret, pseudo) VALUES (?,?,?,?)');
        $req->execute(array($email, $password, $secret, $pseudo));
